...
def test_model():
    ...
    class Model(tiny.nn.Module):
        def __init__(self):
            super().__init__()
            self.l1 = tiny.nn.Linear(3, 4, name='l1')
            self.l2 = tiny.nn.Linear(4, 1, name='l2')
    ...
    optimizer = tiny.optim.SGD(model.parameters(), lr=1e-2, weight_decay=1e-3)
    scheduler = tiny.optim.lr_scheduler.LinearLR(optimizer, start_factor=1.0, end_factor=0.5, total_iters=100)
    ...
    for i in range(1000):
        y_hat = model(X)
        y_hatT = modelT(XT)
        loss = tiny.nn.functional.binary_cross_entropy(y_hat, y)
